...
firstopen = open("hata.txt", "w")
with open("butuncihazlar.txt", "r") as text:
...
for line in lines:
 try:
  ...
  cihazyazilacak = soup.find('div',{'class' : 'name h4'}).text
  ...
  with open('cihaz1.txt', 'r', encoding="utf-8") as file :
    filedata = file.read()
  filedata = filedata.replace('[<h3 class="h4">', '')
  with open('cihaz1.txt', 'w', encoding="utf-8") as file:
    file.write(filedata)
  ...
  # try:
    ...
    # cursor.execute("UPDATE `kimovil` SET `Outmodel` = RTRIM('" + outmodelname.strip() + "') Where Modelname = '"+ eklemodel +"'")	
  # except:
  ...
  for line in lines:
    ...
    try:
      ...
      cursor.execute("UPDATE `tidphones` SET `"+ "K"+ekle.strip() +"` = RTRIM('" + ekle2.strip() + "') Where Outmodel = '"+ outmodelname.strip() +"'")
    except: 
        pass
    cnx.commit()
    i+=2
